...
CREATE TABLE IF NOT EXISTS system_settings (
  ...
  barangay_name TEXT DEFAULT 'Barangay Mabiga',
  city_municipality TEXT DEFAULT 'Hermosa',
  province TEXT DEFAULT 'Bataan',
  region TEXT DEFAULT 'Region III - Central Luzon',
  contact_number TEXT,
  email TEXT,
  ...
  map_default_lat DECIMAL(10, 7) DEFAULT 14.8261000,
  map_default_lng DECIMAL(10, 7) DEFAULT 120.5180000,
...
-- INCIDENT_REPORTS policies cleanup
...
DROP POLICY IF EXISTS "reports_staff_delete" ON incident_reports;

-- INCIDENT_IMAGES policies cleanup
...
-- ANNOUNCEMENTS policies cleanup
...
-- ACTIVITY_LOGS policies cleanup
...
DROP POLICY IF EXISTS "logs_insert_any" ON activity_logs;

-- CHAT_HISTORY policies cleanup
...
-- SYSTEM_SETTINGS policies cleanup
...
-- INCIDENT_REPORTS policies with strict role separation
...
CREATE POLICY "reports_select" ON incident_reports 
  FOR SELECT USING (
    CASE 
      ...
      ELSE reporter_id = auth.uid() -- Fallback: users can see their own reports
    END
  );

CREATE POLICY "reports_update" ON incident_reports 
  FOR UPDATE USING (
    CASE 
...
CREATE POLICY "reports_delete" ON incident_reports 
  FOR DELETE USING (get_user_role() IN ('staff', 'admin'));

-- INCIDENT_IMAGES policies with role separation
CREATE POLICY "images_select" ON incident_images 
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "images_insert" ON incident_images 
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM incident_reports ir 
      WHERE ir.id = incident_images.report_id 
      AND (ir.reporter_id = auth.uid() OR get_user_role() IN ('staff', 'admin'))
    )
  );

CREATE POLICY "images_delete" ON incident_images 
  FOR DELETE USING (get_user_role() IN ('staff', 'admin'));
...
CREATE POLICY "notifications_insert" ON notifications 
  FOR INSERT WITH CHECK (
    get_user_role() IN ('staff', 'admin') OR 
    user_id = auth.uid()
  );
...
CREATE POLICY "chat_admin" ON chat_history 
  FOR ALL USING (get_user_role() = 'admin');
...
CREATE POLICY "settings_manage" ON system_settings 
  FOR ALL USING (get_user_role() = 'admin');

-- ============================================
-- STORAGE BUCKETS (Safe creation)
-- ============================================
DO $$ BEGIN
...
-- Storage policies (safe cleanup and creation)
...
CREATE POLICY "images_view" ON storage.objects 
...
-- ============================================
-- REALTIME (Safe addition)
-- ============================================
DO $$ 
BEGIN
  -- Only add tables to realtime if not already present
  IF NOT EXISTS (
...
-- ============================================
SELECT 
  'SMART-Barangay database setup completed successfully!' as message,
  'Role-based access control is now active.' as security_status,
  'You can now create admin accounts at /setup-admin' as next_step;